...
#else
    // ESP-IDF v6.0 uses LCD_RGB_DATA_ENDIAN_BIG/LITTLE instead of LCD_RGB_ENDIAN_RGB/BGR
    switch (panel_dev_config->data_endian) {
    case LCD_RGB_DATA_ENDIAN_BIG:
        nv3023->madctl_val = 0;  // RGB order
        break;
    case LCD_RGB_DATA_ENDIAN_LITTLE:
        nv3023->madctl_val |= LCD_CMD_BGR_BIT;  // BGR order
        break;
    default:
